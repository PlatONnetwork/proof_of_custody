...
inline unsigned BaseInstruction::get_mem(RegType reg_type) const
{
...
inline ostream &operator<<(ostream &s, const Instruction &instr)
{
  s << instr.opcode << " : ";
  for (int i = 0; i < 3; i++)
  {
...
void Program::execute(Processor<sint, sgf2n> &Proc) const
{
  ...
  while (Proc.PC < size)
  {
    ...
    int opcodex = instruction.get_opcode();
    //printf("opcode:%d\n", opcodex);
    switch (opcodex)
    {
#define X(NAME, PRE, CODE)         \
  ...
  break;
      ARITHMETIC_INSTRUCTIONS_x 
    case MULM:
      {
        auto dest = &Procp.get_S()[r[0]];
        auto op1 = &Procp.get_S()[r[1]];
    ...
    case MULC:
    {
      auto dest = &Procp.get_C()[r[0]];
      auto op1 = &Procp.get_C()[r[1]];
      auto op2 = &Procp.get_C()[r[2]];
      for (int i = 0; i < size; i++)
      {
        *dest++ = *op1++ * *op2++;
